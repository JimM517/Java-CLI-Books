...
public class App {
    ...
    private void loadData() {
        ...
        for (String line : dataset) {
            String[] newDataset = line.split(FIELD_DELIMITER);
            titles.add(newDataset[0]);
            authors.add(newDataset[1]);
            publishedYears.add(parseInt(newDataset[2]));
            prices.add(new BigDecimal(newDataset[3]));
        }
...
//                        displayPricesList(prices);
    ...
    /*
     ...
     See README for additional details.
     */
    public void displaySearchResults(List<Integer> indexes) {
    ...
    private List<Integer> findMostRecentBooks() {
        ...
        for (int k = 0; k < publishedYears.size(); k++) {
            if (publishedYears.get(k) == mostRecentYear) {
                results.add(k);
            }
        }
        return results;
    ...
    private List<Integer> findLeastExpensiveBooks() {
        List<Integer> results = new ArrayList<>();
        double min = prices.get(0).doubleValue();
        for (int i = 0; i < prices.size(); i++) {
            if (prices.get(i).doubleValue() < min) {
                min = prices.get(i).doubleValue();
            }
        }

        for (int k = 0; k < prices.size(); k++) {
            if (prices.get(k).doubleValue() == min) {
                results.add(k);
            }
        }
        return results;
    ...
    private int promptForMenuSelection(String prompt) {
        ...
        int menuSelection;
        try {
            menuSelection = parseInt(keyboard.nextLine());
    ...
    private int promptForPublishedYear(String prompt) {
        ...
        while (true) {
            ...
            try {
                year = parseInt(keyboard.nextLine());
                break;
            } catch (NumberFormatException e) {
